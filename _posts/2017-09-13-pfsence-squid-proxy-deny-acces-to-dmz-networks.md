...
Якщо використовувати разом з [pfSence](https://www.pfsense.org/) модуль [Squid proxy server](https://wiki.squid-cache.org/) то існує можливість потрапляти користувачам до деяких [DMZ](javascript:void(0)) локальних мереж.  
Використовуючи локальні адреси роутера але через процес squid.  
Як вихід додав до дозвіл до [DMZ](javascript:void(0)) мережі тільки спеціальному користувачу  який пройде автентифікацію на у проксі сервері, якщо така (proxy\_auth) використовується.  
